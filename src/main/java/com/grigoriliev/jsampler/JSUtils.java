...
public class JSUtils {
	 ...
	 * @see JSMainFrame#installJSamplerHome
	 */
	public static void
	checkJSamplerHome() {
	...
	changeJSamplerHome(String path) {
		...
		if(fNew.exists() && fNew.isFile()) {
			CC.getViewConfig().showErrorMessage(JSI18n.i18n.getError("CC.JSamplerHomeIsNotDir!"));
			return;
		}
		...
		if(!fNew.exists()) {
			if(!fNew.mkdir()) {
				...
				CC.getViewConfig().showErrorMessage(JSI18n.i18n.getError("CC.mkdirFailed", s));
				return;
			}
		}
	...
	exportInstrumentsToLscpScript(int mapId, MidiInstrumentMap map, Client lscpClient)
		...
		boolean b = CC.preferences().getBoolProperty(JSPrefs.LOAD_MIDI_INSTRUMENTS_IN_BACKGROUND);
	...
	exportInstrumentsToText(MidiInstrumentMap map, StringBuffer sb) {
		int bank = -1;
		String nl = System.getProperty("line.separator");
		int bnkOffset = CC.preferences().getIntProperty(JSPrefs.FIRST_MIDI_BANK_NUMBER);
		int prgOffset = CC.preferences().getIntProperty(JSPrefs.FIRST_MIDI_PROGRAM_NUMBER);
	...
	exportSessionToLscpScript() {
		...
		boolean b = CC.preferences().getBoolProperty(JSPrefs.EXPORT_MIDI_MAPS_TO_SESSION_SCRIPT);
		if(b) {
			exportInstrMapsToLscpScript(lscpClient);
			sb.append(out.toString());
			out.reset();
